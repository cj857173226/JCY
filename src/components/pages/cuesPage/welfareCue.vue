    ...
    <div class="main-body">
      ...
      <div class="cue-filter-wrap">
        <div class="cue-source clearfix">
          ...
          <div class="right">
            <div v-show="welfareList.length>0" class="site-item" :class="{'site-item-on':welfareList == item }" @click="clueWelfareOder(item)" v-for="(item,index) in welfareList" >{{item}}</div>
            <div v-show="welfareList.length==0"> 无 </div>
          </div>
        </div>
...
<script>
  export default {
    ...
    mounted(){
      ...
      _this.getWelfareCueList(); //获取公益组织线索列表
      _this.getWelfare(); //获取来源公益组织
